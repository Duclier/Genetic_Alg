...
const bestFit= () => {
  //let best_fit = 0;
  let obj = {x:0,y:0};
  let best_obj= generation[0];
  let best_fit = generation[0].calcFit()
  //console.log("Começo da func"+best_fit);
  for (var i = 0; i < generation.length; i++) {
    if(generation[i].calcFit() > best_fit){
      ...
      if(generation[i].problem && generation[i].allAcc.length > 5){
        ...
        obj.x = generation[i].pos.x;
        obj.y = generation[i].pos.y;
      }
      //obj.x = generation[i].pos.x+50;
      //obj.y = generation[i].pos.y+50;
      //console.log(obj.x);
      //console.log(obj.x);
      //pos_inicial =[generation[i].pos.x,generation[i].pos.y];
    }
  };

  for (var i = 0; i < best_obj.allAcc.length; i++) {
    traj_objVencedor.push({x:best_obj.allAcc[i].x, y:best_obj.allAcc[i].y});
  };
  for (var i = 0; i < 5; i++) {
    traj_objVencedor.pop()
  };
